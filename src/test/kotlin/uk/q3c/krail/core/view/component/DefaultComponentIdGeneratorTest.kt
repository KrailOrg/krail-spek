    ...
    given("a generator") {
        ...
        given("a view with a single component") {
            ...
            on("generate") {
                ...
                it("applies id to component") {
                    result.nodes().shouldContain(ComponentIdEntry(name = "box1", id = "TestView1-box1", type = "TextField"))
        ...
        given("a view with no components") {
            ...
            on("generate") {
                ...
                it("generates graph with just the view as an entry") {
                    ...
                    result.nodes().shouldContain(ComponentIdEntry(name = "TestView0", id = "TestView0", type = "TestView0"))
                }
            }
        }
        ...
        given("a view with components and layout components") {
            ...
            on("apply") {
                ...
                it("applies id to component") {
                    result.nodes().shouldContain(ComponentIdEntry(name = "box1", id = "TestView2-box1", type = "TextField"))
        ...
        given("a view with all combinations of components") {
            ...
            on("generate and apply") {
                ...
                it("Should have the correct number of total nodes, then only need to check for existence of entries, not absence") {
                    result.nodes().size shouldBe (15)
                }

                it("contains component declared in the View") {
                    val expectedId = "FullMontyView-componentInView"
                    result.nodes().shouldContain(ComponentIdEntry(name = "componentInView", id = expectedId, type = "TextField"))
                ...
                it("does not contain component declared in the View, excluded by annotation") {
                    result.nodes().shouldNotContain(ComponentIdEntry(name = "componentInViewExcluded", id = "FullMontyView-componentInViewExcluded", type = "TextField"))
                    view.componentInViewExcluded.id.shouldBeNull()
                }

                it("does not contain a layout without annotation") {
                    result.nodes().shouldNotContain(ComponentIdEntry(name = "layoutInView", id = "FullMontyView-layoutInView", type = "VerticalLayout"))
                }

                it("generates an id for an empty (not constructed) field, but does not attempt to apply it") {
                    val expectedId = "FullMontyView-emptyField"
                    result.nodes().shouldContain(ComponentIdEntry(name = "emptyField", id = expectedId, type = "TextField"))
                ...
                it("generates an id for an Layout annotated with ('true,false'), but does not drill down") {
                    ...
                    result.nodes().shouldContain(ComponentIdEntry(name = "layoutWithAnnotationOnlyAssign", id = expectedId, type = "SpecificLayout"))
                ...
                it("generates an id for an annotated Layout, but not for its content, with annotation(true,false)") {
                    ...
                    result.nodes().shouldContain(ComponentIdEntry(name = "layoutWithAnnotationOnlyAssign", id = expectedId, type = "SpecificLayout"))
                    view.layoutWithAnnotationOnlyAssign.id shouldBeEqualTo (expectedId)
                    result.nodes().shouldNotContain(ComponentIdEntry(name = "label", id = "$expectedId-label", type = "Label"))
                ...
                it("generated an id for an annotated layout, and its content, with annotation(true,true)") {
                    ...
                    result.nodes().shouldContain(ComponentIdEntry(name = "layoutWithAnnotationAssignAndDrilldown", id = expectedId, type = "SpecificLayout"))
                    view.layoutWithAnnotationAssignAndDrilldown.id shouldBeEqualTo (expectedId)
                    result.nodes().shouldContain(ComponentIdEntry(name = "label", id = "$expectedId-label", type = "Label"))
                ...
                it("generated an id for an annotated layout, and its content, with annotation(false,true), and assigns layout id, overriding annotation.assign") {
                    ...
                    result.nodes().shouldContain(ComponentIdEntry(name = "layoutWithAnnotationOnlyDrilldown", id = expectedId, type = "SpecificLayout"))
                    view.layoutWithAnnotationOnlyDrilldown.label.id.shouldBeEqualTo("FullMontyView-layoutWithAnnotationOnlyDrilldown-label")
                    result.nodes().shouldContain(ComponentIdEntry(name = "label", id = "FullMontyView-layoutWithAnnotationOnlyDrilldown-label", type = "Label"))
                }
                ...
                it("for class annotation (true,true), generated an id and drilled down class annotated composite") {
                    ...
                    result.nodes().shouldContain(ComponentIdEntry(name = "annotatedSpecificLayoutBothTrue", id = expectedId, type = "AnnotatedSpecificLayoutBothTrue"))
                ...
                it("for class annotation (false,true), generates id for contained component, with full includes its immediate parent") {
                    ...
                    result.nodes().shouldContain(ComponentIdEntry(name = "annotatedSpecificLayoutDrilldownOnly", id = expectedId, type = "AnnotatedSpecificLayoutDrilldownOnly"))
                ...
                it("generates an id a field (with annotation(true,true)) declared as an interface, and a field declared by its implementation") {
                    ...
                    result.nodes().shouldContain(ComponentIdEntry(name = "declaredByInterface", id = expectedId, type = "SpecialComponentInterface"))
                    result.nodes().shouldContain(ComponentIdEntry(name="content", id="FullMontyView-declaredByInterface-content", type="Component"))
